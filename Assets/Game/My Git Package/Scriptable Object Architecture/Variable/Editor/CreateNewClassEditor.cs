...
public class CreateNewClassEditor : EditorWindow
{
    ...
    /// </summary>
    private string pathToYourScriptTemplate = "";
    ...
    void OnGUI()
    {
        ...
        var generatedCode = AssetDatabase.FindAssets("Generated Code");
        _path = string.Concat(AssetDatabase.GUIDToAssetPath(generatedCode[0]), "/");

        _path = EditorGUILayout.TextField("Path", _path);

        var ids = AssetDatabase.FindAssets("GenericVariable.cs");
        pathToYourScriptTemplate = AssetDatabase.GUIDToAssetPath(ids[0]);
...
public class CustomAssetModificationProcessor : UnityEditor.AssetModificationProcessor
{
    static void OnWillCreateAsset(string path)
    {
        ...
        file = file.Replace("#VARIABLENAME#",
            string.Format("{0}-{1}", "Variable", SO_Utility.FirstCharToUpper(CreateNewClassEditor.TypeName)));
